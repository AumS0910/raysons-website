...
function updateEmbers(ctx, arr, w, h, progress) {
  ...
  if (arr.length === 0) {
    const count = isMobile()
      ? (isLowPerf() ? 24 : isIOS() ? 46 : 34)
      : 60;
    for (let i = 0; i < count; i++) arr.push(new Ember(w, h, true));
  }
  ctx.clearRect(0, 0, w, h);
  if (progress < .01) return;
  const emberIntensity = isMobile()
    ? Math.min(1, Math.max(.34, progress * 1.18))
    : progress;
  arr.forEach(ember => {
    ember.update(w, h);
    ember.draw(ctx, emberIntensity);
  });
}
